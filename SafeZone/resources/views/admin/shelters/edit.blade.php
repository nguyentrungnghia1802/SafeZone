...
<div class="max-w-3xl mx-auto py-8">
    ...
    <div class="bg-gradient-to-br from-emerald-900 to-emerald-700 rounded-2xl shadow-lg p-6 mb-8">
        ...
        <div id="map-selected-preview" class="mt-2 text-sm text-emerald-200 italic">{{ $shelter->address }}</div>
    </div>

    <!-- Shelter Form Card -->
    <div class="bg-gray-900 rounded-2xl shadow-lg p-8">
        <form method="POST" action="{{ route('admin.shelters.update', $shelter) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <div class="flex gap-2 items-center">
                    <input type="text" id="address_display" required class="flex-1 px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" value="{{ $shelter->address }}" readonly />
            ...
            <input type="hidden" name="address" id="address" value="{{ $shelter->address }}">
            <input type="hidden" name="latitude" id="latitude" value="{{ $shelter->latitude }}">
            <input type="hidden" name="longitude" id="longitude" value="{{ $shelter->longitude }}">

            <!-- Name -->
            <div class="mb-6">
                <label for="name" class="block text-gray-300 font-semibold mb-2">Shelter Name</label>
                <input type="text" name="name" id="name" required value="{{ old('name', $shelter->name) }}" class="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none">
            </div>

            <!-- Description -->
            <div class="mb-6">
                <label for="description" class="block text-gray-300 font-semibold mb-2">Description</label>
                <textarea name="description" id="description" class="w-full h-[100px] px-4 py-2 border bg-gray-800 text-white rounded-lg focus:ring focus:ring-emerald-200 focus:outline-none">{{ old('description', $shelter->description) }}</textarea>
            </div>

            <!-- Capacity -->
            <div class="mb-6">
                <label for="capacity" class="block text-gray-300 font-semibold mb-2">Capacity (people)</label>
                <input type="number" name="capacity" id="capacity" required value="{{ old('capacity', $shelter->capacity) }}" min="0" class="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none">
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="type" name="type" required class="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    <option value="general" {{ old('type', $shelter->type) == 'general' ? 'selected' : '' }}>General</option>
                    <option value="medical" {{ old('type', $shelter->type) == 'medical' ? 'selected' : '' }}>Medical</option>
            ...
            <div class="mb-6">
                ...
                <select id="status" name="status" required class="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    ...
                    <option value="full" {{ old('status', $shelter->status) == 'full' ? 'selected' : '' }}>Full</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="contact_phone" id="contact_phone" value="{{ old('contact_phone', $shelter->contact_phone) }}" class="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                </div>
                <div>
                    <label for="contact_email" class="block text-gray-300 font-semibold mb-2">Contact Email</label>
                    <input type="email" name="contact_email" id="contact_email" value="{{ old('contact_email', $shelter->contact_email) }}" class="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-2 gap-3">
                    @php
                        $facilities = old('facilities', $shelter->facilities ?? []);
                    @endphp
                    <label class="flex items-center gap-2 text-gray-300">
                        <input type="checkbox" name="facilities[]" value="water" {{ in_array('water', $facilities) ? 'checked' : '' }} class="rounded bg-gray-800 border-gray-600">
                        <span>Water Supply</span>
                    </label>
                    <label class="flex items-center gap-2 text-gray-300">
                        <input type="checkbox" name="facilities[]" value="food" {{ in_array('food', $facilities) ? 'checked' : '' }} class="rounded bg-gray-800 border-gray-600">
                        <span>Food</span>
                    </label>
                    <label class="flex items-center gap-2 text-gray-300">
                        <input type="checkbox" name="facilities[]" value="medical" {{ in_array('medical', $facilities) ? 'checked' : '' }} class="rounded bg-gray-800 border-gray-600">
                        <span>Medical Aid</span>
                    </label>
                    <label class="flex items-center gap-2 text-gray-300">
                        <input type="checkbox" name="facilities[]" value="electricity" {{ in_array('electricity', $facilities) ? 'checked' : '' }} class="rounded bg-gray-800 border-gray-600">
            ...
            <div class="mb-6">
                <label class="block text-gray-300 font-semibold mb-2">Current Image</label>
                <img src="{{ asset('storage/' . $shelter->image_path) }}" alt="Shelter Image" class="rounded-lg shadow-md max-w-xs">
            </div>
            @endif

            <!-- Image Upload -->
            <div class="mb-6">
                <label for="image" class="block text-gray-300 font-semibold mb-2">Update Image (optional)</label>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (window.leafletMap) {
            window.leafletCurrentMarker = L.marker([shelterLat, shelterLng]).addTo(window.leafletMap);
            window.leafletMap.setView([shelterLat, shelterLng], 15);
        }
